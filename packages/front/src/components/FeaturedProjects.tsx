...
type FeaturedItem =
  | { type: "corp"; companyId: string; projectId: string }
  | { type: "ai"; projectId: string };

const featured: FeaturedItem[] = [
  { type: "corp", companyId: "santa", projectId: "santa-auth" },
  { type: "ai", projectId: "agent-orchestration" },
  { type: "ai", projectId: "prompt-benchmark" },
  { type: "corp", companyId: "flyingcat", projectId: "fc-app" },
];
        ...
        <div className="grid sm:grid-cols-2 gap-4 md:gap-6">
          {featured.map((item) => {
            if (item.type === "corp") {
              ...
              const project = company.projects.find((p) => p.id === item.projectId)!;
              return (
                <Link
                  key={project.id}
                  ...
                  {project.result && project.result.length > 0 && (
                    <ul className="space-y-1">
                      {project.result.slice(0, 2).map((r, i) => (
                        <li key={i} className="text-xs text-emerald-600 flex gap-1.5 items-start">
              ...
              const aiProject = aiProjects.find((p) => p.id === item.projectId)!;
              return (
                <Link
                  key={aiProject.id}
                  href={`/ai/${aiProject.id}`}
                  className="group p-5 md:p-6 bg-surface rounded-xl border border-border hover:border-primary/30 hover:shadow-md transition-all"
                >
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  {aiProject.result && aiProject.result.length > 0 && (
                    <ul className="space-y-1">
                      {aiProject.result.slice(0, 2).map((r, i) => (
                        <li key={i} className="text-xs text-emerald-600 flex gap-1.5 items-start">
                          <span className="mt-0.5 shrink-0">→</span>
                          {r}
                        </li>
