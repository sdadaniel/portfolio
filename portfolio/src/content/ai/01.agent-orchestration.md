...
- **개선 주도권**: 사소한 개선도 **매번 질문·지시**가 있어야 이어지고, 스스로 이어 가기 어려움
- **컨텍스트·비용**: 사람이 계속 대화를 끌고 가다 보면 **맥락이 길어지고**, 그만큼 **토큰·비용**도 불필요하게 커짐
- **병렬 작업의 충돌**: 여러 agent가 같은 파일을 동시에 수정하면 충돌이 빈번하고, 수동 해결에 시간이 소모됨

## 접근
- Node.js의 event loop와 유사하게 Task Queue를 만들어서 완료 여부를 떠나 task를 지속적으로 주도록 한다.
	- git worktree를 사용해 다중 태스크를 동시에 격리 환경에서 작업하도록 함
	- scope 충돌 검사로 같은 파일을 건드리는 태스크는 순차 실행하여 머지 충돌 방지
- PR을 작성하지 말고 바로 머지시켜 인간이 agent의 속도에 병목이 되지 않도록 함.
	- 이를 위해 task 작업 후 필수적으로 AI review 단계를 거쳐 머지되도록 함
	- review 실패 시 자동 retry, retry 한도 초과 시 failed 처리
	- 24시간 돌리려면 인간의 개입은 최소화되어야 하고 방향성만을 제시해야 함
- ESLint, TypeScript 타입오류, 코드 품질 등은 Night Worker가 24시간 자동으로 점검하고 태스크를 생성

## 기술 스택

- **웹 대시보드**: Next.js, React, TypeScript, Tailwind, Zustand, TanStack Query, Radix, Recharts, xterm.js, WebSocket, Storybook · 테스트(Playwright, Vitest)
- **서버·데이터**: Node.js(tsx), better-sqlite3, node-pty, gray-matter
- **오케스트레이션·런타임**: Bash, Claude CLI, git worktree, SQLite(dual-write), `jq`, `fswatch`, `.orchestration/` 시그널 IPC · 태스크 **마크다운 SoT + DB 캐시**
